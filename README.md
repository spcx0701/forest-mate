...
Android 사용자는 GitHub Release에서 `forestmate-android-vX.apk`를 내려받아 설치할 수 있습니다. Galaxy Watch/Wear OS 사용자는 같은 Release에서 `forestmate-wear-vX.apk`를 설치합니다. F-Droid 제출용 소스 빌드 메타데이터도 포함되며, 현재 Android TWA는 `forestmate.onrender.com` hosted service를 열기 때문에 F-Droid 메타데이터에 `NonFreeNet` Anti-Feature를 명시합니다.
...
├── packaging/            # 배포 빌드 — android(TWA APK·Wear OS APK·Bubblewrap) · ios(Capacitor)
