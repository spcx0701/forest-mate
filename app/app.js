...
function conditionMapMarkup(map) {
  ...
  const legend = (map?.legend ? map.legend : []).map((l) => `<span class="map-legend-item ${esc(l.level)}"><i></i>${esc(l.label)}</span>`).join("");
...
function mntRow(m) {
  const dist = m.dist_km === null || m.dist_km === undefined ? "" : `<div class="loc">🧭 ${m.dist_km}km</div>`;
  return `
